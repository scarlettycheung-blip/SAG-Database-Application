...
DROP FUNCTION IF EXISTS add_car_sale(VARCHAR, VARCHAR, INTEGER, INTEGER, NUMERIC);
DROP FUNCTION IF EXISTS update_car_sale(INTEGER, VARCHAR, VARCHAR, DATE);

CREATE OR REPLACE FUNCTION add_car_sale(make1 VARCHAR, model1 VARCHAR, builtYear INTEGER, odometer INTEGER, price NUMERIC)
RETURNS VOID AS $$
BEGIN
    INSERT INTO CarSales (MakeCode, ModelCode, BuiltYear, Odometer, Price, IsSold)
    VALUES (
        (SELECT MakeCode FROM Make WHERE LOWER(MakeName) = LOWER(make1)),
        (SELECT ModelCode FROM Model WHERE LOWER(ModelName) = LOWER(model1)),
        builtYear, odometer, price, FALSE
    );
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION update_car_sale(carsale_id INTEGER, customer_id VARCHAR, salesperson VARCHAR, sale_date DATE)
RETURNS VOID AS $$
BEGIN
    UPDATE CarSales
    SET BuyerID = customer_id,
        SalespersonID = salesperson,
        SaleDate = sale_date,
        IsSold = TRUE
    WHERE CarSaleID = carsale_id;
END;
$$ LANGUAGE plpgsql;